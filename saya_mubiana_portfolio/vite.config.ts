...
const pkgDeps = Object.keys(pkg.dependencies).filter(d => d !== 'zitejs')
...
const __ziteFlowId = (() => { try { return JSON.parse(readFileSync(path.resolve(__dirname, 'zite.config.json'), 'utf-8')).id ?? ''; } catch { return ''; } })();
